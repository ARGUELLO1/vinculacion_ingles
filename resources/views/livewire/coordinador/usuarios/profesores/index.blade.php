    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            @can('profesores.create')
                <x-primary-button :href="route('coordinador.profesores.create')" wire:navigate>
                    + Crear Profesor
                </x-primary-button>
            @endcan
        </div>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                ...
                <div class="p-6 border-b border-gray-200">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
                        <div class="flex-1">
                            <x-text-input wire:model.live="search" placeholder="Buscar profesores..."
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($profesores as $user)
                                <tr>
                                    <td class="text-center px-6 py-4 whitespace-nowrap">
                                        {{ $user->profesor?->id_profesor ?? 'N/A' }}
                                    </td>
                                    <td class="text-center px-6 py-4 whitespace-nowrap">
                                        {{ $user->profesor?->nombre_completo ?? $user->name }}
                                    </td>
                                    <td class="text-center px-6 py-4 whitespace-nowrap hidden md:table-cell">
                                        {{ $user->profesor?->estatus ?? '' }}
                                    </td>
                                    <td
                                    ...
                                    @can('profesores.options')
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div
                                                ...
                                                @can('profesores.update')
                                                    <x-secondary-button :href="route('coordinador.profesores.edit', $user->profesor)" wire:navigate>
                                                        Editar
                                                    </x-secondary-button>
                                                @endcan

                                                @can('profesores.delete')
