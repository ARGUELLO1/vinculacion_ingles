...
class ActasForm extends Form
{
        ...
        try {
            ...
            foreach ($this->files as $file) {
                if ($alumno) {
                    ...
                    $uploadedCount += $this->subirActaParaAlumno($file, $alumno);
                } else {
                    ...
                    if ($alumnoEncontrado) {
                        $uploadedCount += $this->subirActaParaAlumno($file, $alumnoEncontrado);
                    } else {
                        // Guardar en carpeta de no asignados
                        $rutaNoAsignado = $this->subirActasNoAsignada($file);
                        $noAsignados[] = [
            ...
            $mensaje = $uploadedCount . ' actas(s) subida(s) correctamente.';
     ...
     */
    private function subirActasNoAsignada($file)
    {
    ...
    private function generarRutaNoAsignados()
    {
        ...
        return "Nivel_{$nivel}_Grupo_{$grupo}/Actas/No_Asignados";
    }
    ...
    private function subirActaParaAlumno($file, $alumno)
    {
        // Generar nombre único para el archivo (mantener formato consistente)
        $nombreArchivo = 'acta_' . $this->nivel->nivel . '.pdf';
    ...
    public function eliminarTodasActasAlumno($alumnoId)
    {
        try {
            ...
            return [
                'success' => true,
                'message' => $deletedCount . ' actas(s) eliminada(s) correctamente del alumno.'
            ];
        } catch (\Exception $e) {
            return [
                'success' => false,
                'message' => 'Error al eliminar las actas: ' . $e->getMessage()
            ];
        }
    }

    public function eliminarTodasActasNivel()
    {
        try {
            $documentos = DocumentoNivel::where('nivel_id', $this->nivelId)
                ->where('tipo_doc', 'acta')
            ...
            return [
                'success' => true,
                'message' => $deletedCount . ' actas(s) eliminada(s) correctamente del nivel.'
            ];
        } catch (\Exception $e) {
            return [
                'success' => false,
    ...
    public function getAlumnosConConteo()
    {
        ...
        foreach ($expedientes as $expediente) {
            if ($expediente->alumno) {
                ...
                $rutaCarpetaAlumno = $this->generarRutaCarpeta($alumno);
                $conteoActas = DocumentoNivel::where('nivel_id', $this->nivelId)
                    ->where('tipo_doc', 'acta')
                    ->where('ruta_doc', 'like', $rutaCarpetaAlumno . '%')
                    ->count();
    ...
    public function getConteoActasNivel()
    {
    ...
    public function getConteoActasAlumno($alumnoId)
    {
        $alumno = Alumno::find($alumnoId);
        if (!$alumno) return 0;
    ...
    public function getActasAlumno($alumnoId)
    {
        $alumno = Alumno::find($alumnoId);
    ...
    private function generarRutaCarpeta($alumno = null)
    {
        ...
        $rutaBase = "Nivel_{$nivel}_Grupo_{$grupo}/Actas";
